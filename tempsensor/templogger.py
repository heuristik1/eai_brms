...
mode = 0x5d            # Register 0x01 mode select. V1, V2, V3, V4
changetype = 0x07      #flip to temperature mode
try:
...
def temperature(msb,lsb):
  ...
  temp = msb + lsb
  temp = int(temp, 2)/16
  print "temp msb: %s" %msb[3:]
  print "temp lsb: %s" %lsb
...
#print "T-int: %s Celsius" %temperature(r4,r5)
print "TR1: %s Celsius" %temperature(r6,r7)
print "TR2: %s Celsius" %temperature(ra,rb)
